...
/**
 * Reads a all up to 16 input pins
 * @param hdev
 * @param regData
 * @return
 */
HAL_StatusTypeDef pca9555_WholeInputRead(PCA9555_HandleTypeDef *hdev, uint16_t *regData)
{
	HAL_StatusTypeDef ret = pca9555_readRegister(hdev, PCA9555_CB_INPUTS_PORTS, &regData);
	if (ret != HAL_OK) return ret;
	return HAL_OK;
}
